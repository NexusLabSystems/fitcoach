...
export default function PaymentsPage() {
  ...
  async function handleDelete(payment) {
    if (!window.confirm(`Excluir cobrança de ${studentMap[payment.studentId]?.name ?? "aluno"}? Esta ação não pode ser desfeita.`)) return;
    try {
            ...
            return (
              <div key={payment.id} className="flex items-center gap-4 px-6 py-4 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => handleDelete(payment)} className="p-1.5 rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50 transition-colors" title="Excluir cobrança">
